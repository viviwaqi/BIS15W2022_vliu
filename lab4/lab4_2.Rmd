...
```{r}
filter(fish, lakeid == "AL" | length > 350)
#includes values that are not AL but have a length greater than 350?
```
...
```{r}
mammals_fgsmg_selected <- data.frame(select(mammals, family, genus, species, mass, gestation))
mammals_fgsmg_selected
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
filter(mammals_fgsmg_selected, (family == "Bovidae" & mass > 450000))
```
...
```{r}
mammals_primate_frame <- data.frame(filter(mammals,
 genus == "Lophocebus" | genus == "Erythrocebus" | genus == "Macaca"))
mammals_primate_frame
```
```{r}
primates_mgn_selected <- data.frame(select(mammals_primate_frame,
  mass, gestation, newborn))
primates_mgn_selected
```
```{r}
which.min(mammals_primate_frame$newborn)
min_newborn<- mammals_primate_frame$species [5]
min_newborn
```
