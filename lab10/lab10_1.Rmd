...
```
Using "geom_bar" assumes that y axis is count, so dont have to specify?
...
```{r}
options(scipen=999) #cancels scientific notation for the session, numbers calculated default to being in scientific notation
```
...
```
"reorder" arranges the bars from largest to smallest neatly
## Scatterplots
...
```{r}
elephants %>%
  ggplot(aes(x=age, y=sex))+geom_boxplot()
```
...
elephants %>% 
  ...
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
Pink and blue are the default colors
...
homerange %>%
  ...
  labs(title = "Counts of Endo/Ectotherms",
       x = "Thermoregulation Method",
       y = "Counts") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
```{r}
homerange %>%
  ggplot(aes(x=log10.hra, y=log10.mass, size=log10.mass))+
  geom_point()+
  labs(title = "Homerange vs. Body Mass",
       x = "Homerange (in log10)",
       y = "Body Mass (in log10)")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
can use "color=" the color of the dots, can use "alpha=" to adjust the transparency of the dots
